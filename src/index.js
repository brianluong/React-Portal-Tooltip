...
class Card extends React.Component {
    ...
    getStyle(position, arrow) {
        ...
        let grid = document.getElementById('showgrid').getBoundingClientRect();
        let parentDiv = document.getElementById(this.props.parentEl).getBoundingClientRect();
        let visibleTop, visibleBottom;

        if (grid.top > parentDiv.top) {
            visibleTop = grid.top;
        } else {
            visibleTop = parentDiv.top;
        }

        if (grid.bottom < parentDiv.bottom) {
            visibleBottom = grid.bottom;
        } else {
            visibleBottom = parentDiv.bottom;
        }
        ...
        switch (position) {
            ...
            case 'right':
                style.top = (visibleTop + visibleBottom / 2) - ((this.state.height) / 2)
    ...
    getStyle(position, arrow) {
        ...
        let grid = document.getElementById('showgrid').getBoundingClientRect();
        let parentDiv = this.props.parentEl.getBoundingClientRect();
        let visibleTop, visibleBottom;
    ...
    componentWillReceiveProps() {
        this.setState({transition: this.props.active ? 'all' : 'opacity'}, () => {
            this.updateSize()
        })
    }
    updateSize() {
...
export default class ToolTip extends React.Component {
    ...
    componentDidMount() {
       ...
       document.getElementById('showgrid').addEventListener('scroll', this.onScrollEventListener);
    }
    ...
    componentWillUnmount() {
        ...
        document.getElementById('showgrid').removeEventListener('scroll', this.onScrollEventListener);
    }
    createPortal() {
